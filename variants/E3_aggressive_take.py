...
# E3 variant: take ALL orders where price < fair (not just fair - 1)
# i.e., buy at ask < fair (strictly), sell at bid > fair (strictly)
EMERALDS_TAKE_WIDTH = 0        # was 1 — now takes at strictly less than fair
...
class Trader:
    ...
    def trade_emeralds(self, state: TradingState, trader_data: dict):
        ...
        # E3: Take at strictly < fair (not fair - 1). Captures the ask=9999 level same as
        # baseline, but also takes ask=10000 fills only if there's crossed market.
        # Key difference: buy at ask < fair (strict < not <=); sell at bid > fair (strict >)
        for ask_price in sorted(order_depth.sell_orders.keys()):
            if ask_price >= fair:   # was: ask_price > fair - TAKE_WIDTH (fair - 1 = 9999)
                break
        ...
        for bid_price in sorted(order_depth.buy_orders.keys(), reverse=True):
            if bid_price <= fair:   # was: bid_price < fair + TAKE_WIDTH (fair + 1 = 10001)
                break

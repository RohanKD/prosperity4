...
# T2 variant: max-volume mid (YBansal 10th place P3)
# Use the price with the maximum resting volume on each side as the fair value anchor
...
class Trader:
    ...
    def trade_tomatoes(self, state: TradingState, trader_data: dict):
        ...
        # T2: Max-volume mid — anchor to prices with deepest resting volume
        # This gives a better estimate of where market makers are parked
        ...
        mid = (max_bid_price + max_ask_price) / 2
        ...
        # Use max-volume mid for fair value (no EMA blend - raw max-vol signal)
        fair = round(mid)

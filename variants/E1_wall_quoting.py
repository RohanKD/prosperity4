...
# E1 variant: front-run large resting orders (walls)
EMERALDS_WALL_THRESHOLD = 15    # volume >= this is a "wall"
...
class Trader:
    ...
    def trade_emeralds(self, state: TradingState, trader_data: dict):
        ...
        # E1 variant: detect walls and quote just inside them
        wall_bids = {p: v for p, v in order_depth.buy_orders.items()
                     if v >= EMERALDS_WALL_THRESHOLD and p < fair - EMERALDS_DISREGARD_EDGE}
        wall_asks = {p: v for p, v in order_depth.sell_orders.items()
                     if -v >= EMERALDS_WALL_THRESHOLD and p > fair + EMERALDS_DISREGARD_EDGE}

        if wall_bids:
            best_wall_bid = max(wall_bids.keys())
            bid_price = best_wall_bid + 1  # quote just above the wall
        else:
            # Normal quoting
        ...
        if wall_asks:
            best_wall_ask = min(wall_asks.keys())
            ask_price = best_wall_ask - 1  # quote just below the wall
        else:
        ...
        # Clamp to avoid quoting at or through fair

...
# T1 variant: log-return mean reversion fair value (JackRao 19th place P3)
# theo = mid * exp(-beta * log_return) - gamma * position
TOMATOES_LOG_RETURN_BETA = -0.2933
TOMATOES_POSITION_GAMMA = 0.012
...
class Trader:
    ...
    def trade_tomatoes(self, state: TradingState, trader_data: dict):
        ...
        # Volume-filtered mid (same as baseline)
        ...
        # T1: log-return mean reversion fair value
        # theo = mid * exp(beta * log_return) - gamma * position
        # beta is negative (-0.2933), so when price goes up (positive log_return),
        # theo is lower than mid (predicting reversion)
        if prev_mid > 0 and mid > 0:
            log_return = math.log(mid / prev_mid)
            theo = mid * math.exp(TOMATOES_LOG_RETURN_BETA * log_return) - TOMATOES_POSITION_GAMMA * position
        else:
            theo = mid

        fair = round(theo)

        # Keep EMA trend detection for clearing logic

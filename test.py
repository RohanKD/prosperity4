...
EMERALDS_TAKE_WIDTH = 2        # take orders mispriced by >= this many ticks
EMERALDS_CLEAR_WIDTH = 1       # clear inventory at fair_value +/- this
...
EMERALDS_DEFAULT_EDGE = 3      # default quote distance from fair value
EMERALDS_SOFT_LIMIT = 40       # start inventory skewing here
...
class Trader:
    ...
    def trade_emeralds(self, state: TradingState, trader_data: dict):
        ...
        # Track daily price extrema (useful for bot-tracking in later rounds)
    ...
    def trade_tomatoes(self, state: TradingState, trader_data: dict):
        ...
        # Wall-based mid: anchor fair value to the deepest liquidity levels
        big_bids = {p: v for p, v in order_depth.buy_orders.items() if v >= 20}
        big_asks = {p: v for p, v in order_depth.sell_orders.items() if -v >= 20}
        if big_bids and big_asks:
            wall_bid = max(big_bids, key=lambda p: big_bids[p])
            wall_ask = min(big_asks, key=lambda p: -big_asks[p])
            mid = (wall_bid + wall_ask) / 2
        else:
        ...
        # ── Spike detection: fade moves > 3 std dev from 10-bar window ──
        if len(prices) >= 10:
            window = prices[-10:]
            wmean = sum(window) / len(window)
            wstd = math.sqrt(sum((p - wmean) ** 2 for p in window) / len(window))
            if wstd > 0 and abs(mid - wmean) > 3 * wstd:
                ...
                if mid > wmean:
                    # Price spiked up — sell aggressively into the spike
                    can_sell = limit + (position - 0)
                    qty = min(20, can_sell)
                    if qty > 0:
                        orders.append(Order(product, best_bid, -qty))
                else:
                    # Price spiked down — buy aggressively into the spike
